...
public class GeneralLedgerController {
    ...
    public ResponseEntity<ApiResponse<GlAccountResponse>> createGlAccount(@RequestBody CreateGlAccountRequest request) {
    ...
    @PreAuthorize("hasAnyRole('CBS_ADMIN','CBS_OFFICER')")
    public ResponseEntity<ApiResponse<JournalEntryResponse>> postJournal(@RequestBody PostJournalRequest request) {
        return ResponseEntity.status(HttpStatus.CREATED).body(ApiResponse.ok(toJournalResponse(glService.postJournal(request), true)));
    }

    @GetMapping("/journals/{id}")
    @PreAuthorize("hasAnyRole('CBS_ADMIN','CBS_OFFICER','CBS_VIEWER')")
    public ResponseEntity<ApiResponse<JournalEntryResponse>> getJournal(@PathVariable Long id) {
        return ResponseEntity.ok(ApiResponse.ok(toJournalResponse(glService.getJournal(id), true)));
    }
    ...
    public ResponseEntity<ApiResponse<JournalEntryResponse>> reverseJournal(@PathVariable Long id) {
        return ResponseEntity.ok(ApiResponse.ok(toJournalResponse(glService.reverseJournal(id), true)));
    }
    ...
    public ResponseEntity<ApiResponse<List<JournalEntryResponse>>> getJournalsByDate(
        ...
        return ResponseEntity.ok(ApiResponse.ok(result.getContent().stream().map(journal -> toJournalResponse(journal, false)).toList(), PageMeta.from(result)));
    }
    ...
    public ResponseEntity<ApiResponse<SubledgerReconRunResponse>> runRecon(
        ...
        return ResponseEntity.ok(ApiResponse.ok(toSubledgerReconRunResponse(glService.runReconciliation(subledgerType, glCode, subledgerBalance, reconDate))));
    }
    ...
    private JournalEntryResponse toJournalResponse(JournalEntry journal, boolean includeLines) {
    ...
    private SubledgerReconRunResponse toSubledgerReconRunResponse(SubledgerReconRun run) {
        return new SubledgerReconRunResponse(
                run.getId(),
                run.getReconDate(),
                run.getSubledgerType(),
                run.getGlCode(),
                run.getGlBalance(),
                run.getSubledgerBalance(),
                run.getDifference(),
                run.getIsBalanced(),
