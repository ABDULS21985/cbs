...
public class CapitalMarketsController {
    ...
    @GetMapping("/deals") @PreAuthorize("hasAnyRole('CBS_ADMIN','CBS_OFFICER')") public ResponseEntity<ApiResponse<List<CapitalMarketDeal>>> listAllDeals() { return ResponseEntity.ok(ApiResponse.ok(service.getAllDeals())); }
    @PostMapping("/deals") @PreAuthorize("hasRole('CBS_ADMIN')") public ResponseEntity<ApiResponse<CapitalMarketDeal>> createDeal(@RequestBody CapitalMarketDeal deal) { return ResponseEntity.status(HttpStatus.CREATED).body(ApiResponse.ok(service.createDeal(deal))); }
    @PutMapping("/deals/{id}") @PreAuthorize("hasRole('CBS_ADMIN')") public ResponseEntity<ApiResponse<CapitalMarketDeal>> updateStage(@PathVariable Long id, @RequestParam String stage) { return ResponseEntity.ok(ApiResponse.ok(service.updateStage(id, stage))); }
