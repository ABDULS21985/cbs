...
public class IvrController {
    ...
    @PostMapping("/sessions") @PreAuthorize("hasAnyRole('CBS_ADMIN','CBS_OFFICER')") public ResponseEntity<ApiResponse<IvrSession>> startSession(@RequestParam String callerNumber, @RequestParam(required = false) Long customerId) { return ResponseEntity.status(HttpStatus.CREATED).body(ApiResponse.ok(ivrService.startSession(callerNumber, customerId))); }
    @PostMapping("/sessions/{sessionId}/navigate") @PreAuthorize("hasAnyRole('CBS_ADMIN','CBS_OFFICER')") public ResponseEntity<ApiResponse<IvrSession>> navigate(@PathVariable String sessionId, @RequestParam String option) { return ResponseEntity.ok(ApiResponse.ok(ivrService.navigate(sessionId, option))); }
    @PostMapping("/sessions/{sessionId}/transfer") @PreAuthorize("hasAnyRole('CBS_ADMIN','CBS_OFFICER')") public ResponseEntity<ApiResponse<IvrSession>> transfer(@PathVariable String sessionId, @RequestParam String reason) { return ResponseEntity.ok(ApiResponse.ok(ivrService.transferToAgent(sessionId, reason))); }
}
