...
import com.cbs.commission.dto.CalculatePayoutRequest;
...
public class CommissionController {
    ...
    @GetMapping("/agreements") @PreAuthorize("hasAnyRole('CBS_ADMIN','CBS_OFFICER')") public ResponseEntity<ApiResponse<List<CommissionAgreement>>> listAgreements() { return ResponseEntity.ok(ApiResponse.ok(service.getAllAgreements())); }
    @GetMapping("/agreements/{code}") @PreAuthorize("hasAnyRole('CBS_ADMIN','CBS_OFFICER')") public ResponseEntity<ApiResponse<CommissionAgreement>> getAgreement(@PathVariable String code) { return ResponseEntity.ok(ApiResponse.ok(service.getAgreementByCode(code))); }
    ...
    @PostMapping("/agreements/{code}/calculate-payout") @PreAuthorize("hasRole('CBS_ADMIN')") public ResponseEntity<ApiResponse<CommissionPayout>> calculatePayout(@PathVariable String code, @Valid @RequestBody CalculatePayoutRequest request) { return ResponseEntity.status(HttpStatus.CREATED).body(ApiResponse.ok(service.calculatePayout(code, request.getGrossSales(), request.getQualifyingSales(), request.getPeriod()))); }
