...
public class CardDisputeController {
    ...
    public ResponseEntity<ApiResponse<CardDispute>> initiate(
            ...
            @RequestParam String cardScheme, @RequestParam String createdBy) {
        return ResponseEntity.status(HttpStatus.CREATED).body(ApiResponse.ok(disputeService.initiateDispute(
                ...
                disputeType, disputeReason, disputeAmount, cardScheme, createdBy)));
    }
    ...
    @PreAuthorize("hasAnyRole('CBS_ADMIN','CBS_OFFICER')")
    public ResponseEntity<ApiResponse<CardDispute>> provisionalCredit(@PathVariable Long id, @RequestParam String performedBy) {
        return ResponseEntity.ok(ApiResponse.ok(disputeService.issueProvisionalCredit(id, performedBy)));
    }
    ...
    public ResponseEntity<ApiResponse<CardDispute>> fileChargeback(@PathVariable Long id,
            @RequestParam String schemeCaseId, @RequestParam String schemeReasonCode, @RequestParam String performedBy) {
        return ResponseEntity.ok(ApiResponse.ok(disputeService.fileChargeback(id, schemeCaseId, schemeReasonCode, performedBy)));
    }
    ...
    public ResponseEntity<ApiResponse<CardDispute>> representment(@PathVariable Long id,
            @RequestParam String merchantResponse, @RequestParam String performedBy) {
        return ResponseEntity.ok(ApiResponse.ok(disputeService.recordRepresentment(id, merchantResponse, performedBy)));
    }
    ...
    public ResponseEntity<ApiResponse<CardDispute>> arbitration(@PathVariable Long id,
            ...
            @RequestParam String performedBy, @RequestParam(required = false) String notes) {
        return ResponseEntity.ok(ApiResponse.ok(disputeService.escalateToArbitration(id, preArbitration, performedBy, notes)));
    }
    ...
    public ResponseEntity<ApiResponse<CardDispute>> resolve(@PathVariable Long id,
            ...
            @RequestParam(required = false) String notes, @RequestParam String performedBy) {
        return ResponseEntity.ok(ApiResponse.ok(disputeService.resolveDispute(id, resolutionType, resolutionAmount, notes, performedBy)));
    }
