...
export const loanHandlers = [
  ...
  http.get('/api/v1/loans/products', () => HttpResponse.json(wrap([
    {
      id: 1,
      code: 'PERSONAL-001',
      name: 'Personal Loan',
      description: 'Unsecured personal lending product',
      loanType: 'PERSONAL',
      currencyCode: 'NGN',
      minInterestRate: 18,
      maxInterestRate: 24,
  ...
  http.post('/api/v1/loans/schedule-preview', () => HttpResponse.json(wrap(Array.from({ length: 12 }, (_, i) => ({
    ...createMockScheduleItem(i + 1),
    principalPaid: 0,
    interestPaid: 0,
    totalPaid: 0,
  }))))),
