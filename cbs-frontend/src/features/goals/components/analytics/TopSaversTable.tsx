...
export function TopSaversTable({ goals }: Props) {
  ...
  const byCustomer: Record<number, { name: string; goals: SavingsGoal[] }> = {};
  goals.forEach((g) => {
    const key = g.customerId;
    if (!byCustomer[key]) byCustomer[key] = { name: g.customerDisplayName, goals: [] };
    byCustomer[key].goals.push(g);
  });

  const rows: SaverRow[] = Object.entries(byCustomer).map(([id, { name, goals: custGoals }]) => {
    ...
    <div className="rounded-lg border overflow-hidden">
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y">
          {rows.map((r, i) => (
            <tr key={r.customerId} className="hover:bg-muted/20 cursor-pointer" onClick={() => navigate(`/customers/${r.customerId}`)}>
              <td className="px-4 py-2.5 text-sm font-bold text-muted-foreground">{i < 3 ? ['🥇', '🥈', '🥉'][i] : i + 1}</td>
              <td className="px-4 py-2.5 text-sm">{r.customerName}</td>
