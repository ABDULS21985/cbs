...
export const depositAnalyticsApi = {
  getDepositStats: (): Promise<DepositStats> =>
    apiGet<DepositStats>('/v1/reports/deposits/stats'),

  getDepositMix: (): Promise<DepositMixItem[]> =>
    apiGet<DepositMixItem[]>('/v1/reports/deposits/mix'),

  getDepositGrowthTrend: (): Promise<DepositGrowthPoint[]> =>
    apiGet<DepositGrowthPoint[]>('/v1/reports/deposits/growth-trend'),

  getTopDepositors: (): Promise<TopDepositor[]> =>
    apiGet<TopDepositor[]>('/v1/reports/deposits/top-depositors'),

  getMaturityProfile: (): Promise<MaturityBucket[]> =>
    apiGet<MaturityBucket[]>('/v1/reports/deposits/maturity-profile'),

  getRateBands: (): Promise<RateBand[]> =>
    apiGet<RateBand[]>('/v1/reports/deposits/rate-bands'),

  getRateSensitivityData: (): Promise<RateSensitivityPoint[]> =>
    apiGet<RateSensitivityPoint[]>('/v1/reports/deposits/rate-sensitivity'),

  getCostOfFundsTrend: (): Promise<CostOfFundsPoint[]> =>
    apiGet<CostOfFundsPoint[]>('/v1/reports/deposits/cost-of-funds'),

  getRetentionVintage: (): Promise<RetentionVintage[]> =>
    apiGet<RetentionVintage[]>('/v1/reports/deposits/retention-vintage'),

  getChurnAnalysis: (): Promise<ChurnStat> =>
    apiGet<ChurnStat>('/v1/reports/deposits/churn'),
};
