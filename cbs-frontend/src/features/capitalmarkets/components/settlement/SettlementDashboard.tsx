    ...
    <div className="rounded-xl border bg-card p-4">
      <div className="flex items-center justify-between mb-2">
        <p className="text-sm font-medium">
          Today's Settlement:{' '}
          <span className="tabular-nums">{data.totalToday.toLocaleString()}</span> instructions
        </p>
      ...
      <div className="flex items-center gap-6 text-sm">
        <span className="flex items-center gap-1.5 text-green-600">
          ...
          <span className="tabular-nums font-medium">{data.settled}</span> settled
        </span>
        <span className="flex items-center gap-1.5 text-blue-600">
          <Clock className="w-3.5 h-3.5" />
          <span className="tabular-nums font-medium">{data.pending}</span> pending
        </span>
        <span className="flex items-center gap-1.5 text-red-600">
          <AlertTriangle className="w-3.5 h-3.5" />
          <span className="tabular-nums font-medium">{data.failed}</span> failed
        </span>
      </div>
    </div>
  );
