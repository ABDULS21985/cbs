...
function ActionButtons({ item }: { item: DunningQueueItem }) {
  ...
  return (
    <div className="flex gap-1 flex-wrap">
      <button
        onClick={() => log('PROMISE_TO_PAY')}
        disabled={isPending}
        className="px-2 py-0.5 text-xs rounded border border-green-300 text-green-700 hover:bg-green-50 transition-colors disabled:opacity-50"
      >
        Promise to Pay
      </button>
      <button
        ...
        disabled={isPending}
        className="px-2 py-0.5 text-xs rounded border border-gray-300 text-gray-600 hover:bg-gray-50 transition-colors disabled:opacity-50"
      >
        No Answer
      </button>
      <button
        onClick={() => log('SMS_SENT')}
        disabled={isPending}
        className="px-2 py-0.5 text-xs rounded border border-blue-300 text-blue-700 hover:bg-blue-50 transition-colors disabled:opacity-50"
      >
        SMS Sent
      </button>
      <button
        onClick={() => log('SKIPPED')}
        disabled={isPending}
        className="px-2 py-0.5 text-xs rounded border border-amber-300 text-amber-700 hover:bg-amber-50 transition-colors disabled:opacity-50"
      >
        Skip
      </button>
    </div>
