...
import { mockFxRates, mockMoneyMarketRates, mockFeedStatus } from '../api/mockTreasuryData';
            ...
            <tbody>
              {mockFxRates.map((r) => (
                <tr key={r.pair}>
                  ...
                  <td className="px-4 text-sm font-mono text-right">{r.bid.toLocaleString('en', { minimumFractionDigits: 2 })}</td>
                  <td className="px-4 text-sm font-mono text-right">{r.ask.toLocaleString('en', { minimumFractionDigits: 2 })}</td>
                  <td className="px-4 text-sm font-mono text-right font-medium">{r.mid.toLocaleString('en', { minimumFractionDigits: 2 })}</td>
    ...
    <>
      ...
      <div className="page-container">
        <TabsPage syncWithUrl tabs={[
          ...
          { id: 'feeds', label: 'Feed Status', badge: mockFeedStatus.filter((f) => f.status !== 'ONLINE').length || undefined, content: <FeedStatusTab /> },
