...
export function PortalLayout() {
  ...
  return (
    <div className="portal-app-shell min-h-screen">
          ...
          <div className="flex items-center justify-between h-14">
            <div className="flex items-center gap-3">
              <Link to="/portal/dashboard" className="flex items-center gap-2" aria-label="BellBank Home">
                <div className="portal-brand-mark"><span>B</span></div>
                <div className="hidden sm:block">
                  <span className="block text-sm font-semibold text-foreground">BellBank</span>
                  <span className="block text-[11px] text-muted-foreground">Digital portal</span>
                </div>
              </Link>
              {pathSegs.length > 0 && (
                <nav className="hidden md:flex items-center gap-1 rounded-full border px-3 py-1.5 text-xs text-muted-foreground" aria-label="Breadcrumb">
            ...
            <div className="flex items-center gap-2">
              <Link to="/portal/notifications" className="portal-topbar-button relative h-10 w-10" aria-label={`Notifications${unreadCount > 0 ? `, ${unreadCount} unread` : ''}`}>
              ...
              <div className="relative">
                <button onClick={() => setMenuOpen(!menuOpen)} className="portal-topbar-button flex h-10 items-center gap-2 px-2.5 text-sm" aria-expanded={menuOpen}>
                  <div className="flex h-7 w-7 items-center justify-center rounded-full bg-primary/10 text-xs font-semibold text-primary">{(user?.fullName?.[0] || 'U').toUpperCase()}</div>
                  <span className="hidden sm:block text-xs">{user?.fullName?.split(' ')[0] || 'User'}</span>
                  <ChevronDown className="w-3 h-3" />
                </button>
                {menuOpen && (<><div className="fixed inset-0 z-40" onClick={() => setMenuOpen(false)} />
                  <div className="absolute right-0 mt-2 w-56 portal-panel py-1 z-50" role="menu">
      ...
      <div className="max-w-7xl mx-auto flex">
        <aside className="portal-sidebar-shell sticky top-14 overflow-y-auto" role="navigation" aria-label="Portal navigation">
          <nav className="py-4 px-3 space-y-1">
            {SIDEBAR_LINKS.map(l => (<NavLink key={l.path} to={l.path} end={l.path === '/portal/dashboard'}
              className={({ isActive }) => cn('portal-sidebar-link', isActive && 'font-medium')}
        ...
        <main id="portal-main" className="portal-main-shell" role="main">
          <Suspense fallback={<RouteContentLoader />}><Outlet /></Suspense>
        </main>
      </div>

      <nav className="portal-mobile-nav" role="navigation" aria-label="Mobile navigation">
      ...
      {moreOpen && (<div className="md:hidden fixed inset-0 z-50"><div className="absolute inset-0 bg-black/50" onClick={() => setMoreOpen(false)} />
        <div className="portal-mobile-sheet space-y-2 max-h-[60vh] overflow-y-auto">
