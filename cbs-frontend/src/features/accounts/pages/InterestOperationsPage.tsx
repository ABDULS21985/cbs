...
export function InterestOperationsPage() {
  ...
  const accrueMut = useMutation({
    ...
    onError: () => { toast.error('Accrual failed'); setLastResult({ type: 'Accrual', success: false, message: 'Failed to accrue interest' }); },
  });
  ...
  const postMut = useMutation({
    ...
    onError: () => { toast.error('Posting failed'); setLastResult({ type: 'Posting', success: false, message: 'Failed to post interest' }); },
  });
  ...
  const batchMut = useMutation({
    ...
    onError: () => { toast.error('Batch accrual failed'); setLastResult({ type: 'Batch Accrual', success: false, message: 'Batch interest accrual failed' }); },
  });
      ...
      <div className="page-container space-y-6">
        ...
        <div className="bg-card rounded-lg border p-6 space-y-4">
          ...
          <div className="max-w-md space-y-3">
            <div className="space-y-1.5"><label className="text-xs font-medium text-muted-foreground">Account Number or ID *</label>
              <input value={accountId} onChange={e => setAccountId(e.target.value)} placeholder="Enter account number" className={cn(fc, 'font-mono')} /></div>
