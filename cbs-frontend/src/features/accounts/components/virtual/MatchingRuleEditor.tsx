...
const RULE_TYPE_LABELS: Record<MatchingRule['type'], string> = {
...
export function MatchingRuleEditor({ rules: initialRules, onSave }: MatchingRuleEditorProps) {
  ...
  const updateRule = (id: string, field: keyof MatchingRule, value: string | number) => {
    setRules((prev) =>
        ...
        {rules.length === 0 ? (
          ...
          rules.map((rule, idx) => (
            ...
            >
              ...
              <select
                value={rule.type}
                onChange={(e) => updateRule(rule.id, 'type', e.target.value as MatchingRule['type'])}
                className={cn(
                  'text-sm rounded-md border bg-background px-2 py-1.5 focus:outline-none focus:ring-2 focus:ring-primary/50',
                  'w-44 flex-shrink-0',
                )}
              >
                {(Object.keys(RULE_TYPE_LABELS) as MatchingRule['type'][]).map((t) => (
              ...
              <span
                className={cn(
                  ...
                  rule.type === 'REFERENCE_PREFIX' && 'bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400',
                  rule.type === 'REGEX' && 'bg-purple-50 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400',
                  rule.type === 'EXACT' && 'bg-amber-50 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400',
                )}
              >
                {rule.type === 'REFERENCE_PREFIX' ? 'PREFIX' : rule.type === 'REGEX' ? 'REGEX' : 'EXACT'}
              </span>
