        ...
        <div className="bg-card rounded-xl shadow-2xl border w-full max-w-2xl max-h-[90vh] flex flex-col">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b flex-shrink-0">
            <div>
              ...
              <p className="text-sm text-muted-foreground font-mono mt-0.5">{transaction.reference}</p>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className="inline-flex items-center px-2.5 py-1 rounded-md bg-muted text-sm font-medium">
                {transaction.channel} Banking
              </span>
              <StatusBadge status={transaction.status} size="md" dot />
            </div>
              ...
              <div className="grid grid-cols-2 gap-4 p-4 bg-muted/30 rounded-lg">
                {transaction.fromAccount && (
                  <div>
                    ...
                    <p className="font-mono text-sm font-semibold">
                      {transaction.fromAccount.replace(/(\d{4})(\d{4})(\d{2})/, '$1 $2 $3')}
                    </p>
                    {transaction.fromAccountName && (
                      <p className="text-sm text-muted-foreground mt-0.5">{transaction.fromAccountName}</p>
                    )}
                  </div>
                )}
                {transaction.toAccount && (
                  <div>
                    ...
                    <p className="font-mono text-sm font-semibold">
                      {transaction.toAccount.replace(/(\d{4})(\d{4})(\d{2})/, '$1 $2 $3')}
                    </p>
                    {transaction.toAccountName && (
                      <p className="text-sm text-muted-foreground mt-0.5">{transaction.toAccountName}</p>
                    )}
                  </div>
                )}
              </div>
              ...
              {transaction.debitAmount !== undefined && (
                <div>
                  ...
                  <p className="font-mono text-lg font-bold text-red-600 dark:text-red-400">
                    {formatMoney(transaction.debitAmount)}
                  </p>
                </div>
              )}
              {transaction.creditAmount !== undefined && (
                <div>
                  ...
                  <p className="font-mono text-lg font-bold text-green-600 dark:text-green-400">
                    {formatMoney(transaction.creditAmount)}
                  </p>
                </div>
              )}
            ...
            <InfoGrid
              columns={3}
              items={[
                { label: 'Date & Time', value: transaction.dateTime, format: 'datetime' },
                { label: 'Value Date', value: transaction.valueDate, format: 'date' },
                { label: 'Posting Date', value: transaction.postingDate, format: 'date' },
                { label: 'Narration', value: transaction.narration, span: 3 },
              ]}
            />

            {/* GL Entries */}
            {transaction.glEntries && transaction.glEntries.length > 0 && (
              <div>
                ...
                <div className="rounded-lg border overflow-hidden">
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y">
                      {transaction.glEntries.map((entry, i) => (
          ...
          <div className="flex items-center justify-between gap-3 px-6 py-4 border-t flex-shrink-0">
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={() => downloadMutation.mutate(transaction.id)}
            ...
            <div className="bg-card rounded-xl shadow-2xl border w-full max-w-md p-6 space-y-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  You are about to reverse transaction <span className="font-mono font-medium">{transaction.reference}</span>.
              ...
              <div className="flex gap-3 justify-end">
                ...
                <button
                  onClick={() => reverseMutation.mutate({ id: transaction.id, reason: reverseReason })}
